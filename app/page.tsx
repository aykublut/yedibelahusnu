...
export default function QuizApp() {
  ...
  const isLastQuestion = activeQuestionIndex === currentQuestionsList.length - 1;
  ...
  const progressPercentage = currentQuestionsList.length > 0 
  ...
  if (isQuizFinished) {
    ...
    return (
      <div className="h-[100dvh] w-full bg-[#030303] text-slate-200 flex flex-col items-center justify-center p-4 overflow-hidden relative">
        ...
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120vw] h-[120vh] opacity-20 blur-[120px] pointer-events-none transition-colors duration-1000 ${isPerfect ? "bg-[radial-gradient(circle,rgba(16,185,129,0.4)_0%,transparent_60%)]" : "bg-[radial-gradient(circle,rgba(244,63,94,0.4)_0%,transparent_60%)]"}`}></div>
        
        <div className="w-full max-w-md z-10 flex flex-col gap-[clamp(1rem,2dvh,1.5rem)]">
          <div className={`w-full aspect-[16/9] rounded-[2rem] overflow-hidden border-4 relative shrink-0 transition-shadow duration-700 ${isPerfect ? "border-emerald-500/40 shadow-[0_0_40px_rgba(16,185,129,0.25)]" : "border-rose-500/40 shadow-[0_0_40px_rgba(244,63,94,0.25)]"}`}>
             {isPerfect ? (
               <video autoPlay loop playsInline className="absolute inset-0 w-full h-full object-cover pointer-events-none">
             ...
             <div className={`absolute inset-0 mix-blend-overlay opacity-30 pointer-events-none z-10 ${isPerfect ? "bg-emerald-500" : "bg-rose-500"}`}></div>
          </div>
          ...
          <div className="text-center space-y-1 shrink-0">
            ...
            <p className="text-[clamp(0.85rem,1.8dvh,1rem)] font-medium text-slate-400">
              {isPerfect ? "Hıyar yemeyi hak ettin, Şaban seninle gurur duyuyor!" : "Şaban senin bu duruma gülüyor, tekrar denemelisin!"}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            <div className="bg-white/[0.03] border border-emerald-500/20 rounded-3xl p-[clamp(1rem,2dvh,1.5rem)] text-center backdrop-blur-md">
              <span className="text-[clamp(1.8rem,4dvh,2.25rem)] font-black text-emerald-400 block leading-none mb-1">{currentListCorrectCount}</span>
              <span className="text-[10px] font-bold text-emerald-500/70 tracking-widest uppercase">Doğru</span>
            </div>
            <div className="bg-white/[0.03] border border-rose-500/20 rounded-3xl p-[clamp(1rem,2dvh,1.5rem)] text-center backdrop-blur-md">
              <span className="text-[clamp(1.8rem,4dvh,2.25rem)] font-black text-rose-400 block leading-none mb-1">{currentListWrongCount}</span>
              <span className="text-[10px] font-bold text-rose-500/70 tracking-widest uppercase">Yanlış</span>
            </div>
          </div>

          <div className="flex flex-col gap-[clamp(0.5rem,1.2dvh,0.75rem)] shrink-0 mt-2">
            {currentListWrongCount > 0 && (
              <button onClick={retryWrongAnswers} className="w-full py-[clamp(0.85rem,2.2dvh,1rem)] bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-2xl transition-all active:scale-[0.98] shadow-[0_0_25px_rgba(225,29,72,0.3)] flex items-center justify-center gap-2">
                Sadece Yanlışları Çöz
              </button>
            )}
            <button onClick={restartQuiz} className="w-full py-[clamp(0.85rem,2.2dvh,1rem)] bg-white/[0.05] hover:bg-white/[0.1] text-white font-bold rounded-2xl border border-white/10 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
              Sistemi Sıfırla
            </button>
          </div>
        </div>
  ...
  if (qLen > 250) {
    qTextSize = "text-[clamp(0.9rem,1.8dvh,1.1rem)] mb-[clamp(0.5rem,1.5dvh,1.5rem)] leading-snug";
  } else if (qLen > 120) {
    qTextSize = "text-[clamp(1rem,2.2dvh,1.35rem)] mb-[clamp(0.75rem,2dvh,2rem)] leading-tight";
  }
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#050505] text-slate-200 flex flex-col overflow-hidden relative selection:bg-indigo-500/30">
      ...
      <div className="absolute top-0 left-0 w-full h-1 bg-white/5 z-50">
        <div className="h-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.8)] transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }} />
      </div>
      ...
      <div className="w-full max-w-3xl mx-auto px-4 pt-4 sm:pt-6 pb-2 flex items-center justify-between shrink-0 z-10">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`px-2.5 py-1 text-[clamp(10px,1.2dvh,12px)] font-bold tracking-wider uppercase rounded-lg border ${isWrongAnswersMode ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-indigo-500/10 text-indigo-400 border-indigo-500/20"}`}>
          ...
          >
            {isAnalyzing ? (
              <>
                <svg className="w-[14px] h-[14px] animate-spin" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {!isWrongAnswersMode && activeQuestionIndex === 0 && (
            <button onClick={shuffleQuestions} className="px-2.5 py-1 text-[clamp(10px,1.2dvh,12px)] font-bold tracking-wider uppercase rounded-lg border bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20 hover:bg-fuchsia-500/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5 animate-in fade-in slide-in-from-left-2 duration-300">
              <svg className="w-[14px] h-[14px]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="text-[clamp(12px,1.5dvh,14px)] font-semibold text-slate-400">
          <span className="text-white">{activeQuestionIndex + 1}</span> / {currentQuestionsList.length}
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 w-full max-w-3xl mx-auto flex flex-col px-4 pb-4 sm:px-6 z-10">
            ...
            {currentQuestion && (
              <>
                <div className="shrink-0">
                  <h2 className={`font-semibold text-slate-100 tracking-tight transition-all ${qTextSize}`}>
                ...
                <div className="flex flex-col gap-[clamp(0.4rem,1.2dvh,0.8rem)]">
                  {currentQuestion.options.map((option, index) => {
                    ...
                    let baseStyle = "border-white/5 bg-white/[0.03] text-slate-300 hover:border-indigo-500/30 hover:bg-indigo-500/10";
                    let icon = null;

                    if (hasAnswered) {
                      if (isCorrectAnswer) {
                        baseStyle = "border-emerald-500/50 bg-emerald-500/10 text-emerald-300 z-10 scale-[1.02] shadow-[0_0_20px_rgba(16,185,129,0.15)]";
                        icon = <svg className="w-[clamp(1.2rem,2.5dvh,1.5rem)] h-[clamp(1.2rem,2.5dvh,1.5rem)] text-emerald-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7" /></svg>;
                      } else if (isSelected && !isCorrectAnswer) {
                        baseStyle = "border-rose-500/50 bg-rose-500/10 text-rose-300 opacity-90";
                        icon = <svg className="w-[clamp(1.2rem,2.5dvh,1.5rem)] h-[clamp(1.2rem,2.5dvh,1.5rem)] text-rose-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>;
                      } else {
                        baseStyle = "border-transparent bg-white/[0.01] text-slate-600 opacity-30 grayscale";
                      }
                    }

                    const optLen = option.length;
                    let optTextSize = "text-[clamp(0.85rem,2.2dvh,1.1rem)] py-[clamp(0.75rem,2dvh,1.25rem)]";
                    if (optLen > 80) {
                      optTextSize = "text-[clamp(0.75rem,1.6dvh,0.9rem)] py-[clamp(0.5rem,1.5dvh,1rem)] leading-snug";
                    } else if (optLen > 40) {
                      optTextSize = "text-[clamp(0.8rem,1.9dvh,1rem)] py-[clamp(0.6rem,1.8dvh,1.1rem)] leading-snug";
                    }

                    return (
                      <button key={index} disabled={hasAnswered} onClick={() => answerQuestion(currentQuestion.id, option)} className={`group w-full flex items-center justify-between px-[clamp(0.75rem,2dvh,1.25rem)] rounded-[1rem] sm:rounded-[1.25rem] border-2 transition-all duration-300 outline-none text-left shrink-0 ${baseStyle} ${!hasAnswered && "active:scale-[0.98]"}`}>
                        <span className={`font-medium pr-4 break-words ${optTextSize}`}>{option}</span>
                        {icon && <span className="animate-in zoom-in duration-300 shrink-0 pl-2">{icon}</span>}
                      </button>
                    );
                  })}
                </div>
        ...
        <div className="shrink-0 pt-4 pb-2 flex items-center justify-between gap-4">
          <button onClick={prevQuestion} disabled={activeQuestionIndex === 0} className="w-[3.5rem] h-[3.5rem] sm:w-[4rem] sm:h-[4rem] flex items-center justify-center shrink-0 bg-white/[0.05] rounded-full text-slate-400 hover:text-white hover:bg-white/10 disabled:opacity-20 transition-all border border-white/5 active:scale-95">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
          </button>
          {isLastQuestion ? (
            <button onClick={finishQuiz} disabled={!hasAnswered} className="flex-1 h-[3.5rem] sm:h-[4rem] flex items-center justify-center bg-indigo-600 text-white text-[clamp(14px,2dvh,16px)] font-bold rounded-full hover:bg-indigo-500 disabled:opacity-20 transition-all active:scale-[0.98] shadow-lg shadow-indigo-500/20 tracking-wide">
              TARAMAYI BİTİR
            </button>
          ) : (
            <button onClick={nextQuestion} disabled={!hasAnswered} className="flex-1 h-[3.5rem] sm:h-[4rem] flex items-center justify-center gap-2 bg-white text-black text-[clamp(14px,2dvh,16px)] font-bold rounded-full hover:bg-slate-200 disabled:bg-white/10 disabled:text-white/30 transition-all active:scale-[0.98] tracking-wide">
              SONRAKİ
